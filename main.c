...
int coordenadasTableroX[5][5]; //En esta matriz se guardan las coordenadas (X) del tablero, cada lugar de la matriz representa una posici�n del tablero.
int coordenadasTableroY[5][5]; //En esta matriz se guardan las coordenadas (Y) del tablero, cada lugar de la matriz representa una posici�n del tablero.
...
int piezasJ1[12]; //Guarda las posiciones random tomadas para no repetirlas, se utiliza para mandar a una funci�n una pieza aleatoria.
int piezasJ2[12]; //Guarda las posiciones random tomadas para no repetirlas, se utiliza para mandar a una funci�n una pieza aleatoria.
...
int i = 0, j = 0; //�ndices varios para iterar en loops
int fila = 0, columna = 0; //�ndices varios para iterar en loops
int I = 0, J = 0, K = 0, L = 0; //�ndices varios para iterar en loops
...
void FNdimensionarPiezas();//Define el tama�o de las piezas
void FNconfigFormJuego();//Crea la pantalla del juego y el layout del mismo, en dicho layout van colocadas las piezas y el trablero de juego, adem�s otros botones y menus.
...
void FNrandomizarPiezas(); //EN PRUEBA Funci�n que vuelve a randomizar las posiciones iniciales de las piezas de cada jugador.
...
void FNconfigMovimientoPiezas(); //Funcion que configura el Men� de Movimientos de las piezas
void FNcrearMenusPiezas(); //Funci�n que crea los men�s de las piezas
void FNconectarMenusPiezas(); //Funci�n que conecta los men�s a las respectivas piezas
...
void FNmostrarOPmenusPiezas(); //Funci�n particular de la librer�a que hace visible los submen�s para los movimientos.
int  desplegarMenuMovimiento(GtkWidget *widget, GdkEvent *event); //Funcion que despliega el men� PoPup de cada puiza para moverla
...
int siCaeEnTrampa(int fil, int col); //Verifica si el lugar al que se movi� la pieza era una trampa, y si est� a salvo o no de la eliminaci�n de la pieza
...
void FNpiezaClickeada(GtkWidget *pieza); //Funci�n que recibe la pieza en que se hizo click e identifica de que pieza se trata, para luego trabajar con las funciones de movimientos.
...
struct Turnos { //Guarda los objetos de los turnos de los jugadores (Bot�n e imagen del bot�n)
...
struct Juego { //Guarda todos los objetos de cada pieza, el objeto pieza en s� mismo, su imagen, su men�, el peso de cada pieza y sus movimientos. (Se anida con la estrucutra de Movimientos)
    GtkWidget *Pieza;
    int Peso;
    GtkWidget *Imagen;
    GtkWidget *Menu;
...
/// ///////////////////////////// FIN DECLARACI�N DE ESTRUCTURAS DE DATOS UTILIZADAS /////////////////////////////////////////////////////////////////////////////////////////

/// /////////////////////////////////// INICIO FUNCI�N PRINCIPAL MAIN /////////////////////////////////////////////////////////////////////////////////////////////////////////////
 int main(int argc, char *argv[]){


    srand(time(NULL)); //Se inicializan numeros aleatorios
    system("color 1F"); //Da color a la consola
    gtk_init(&argc, &argv); //Se inicializa la librer�a
    GtkSettings *default_settings = gtk_settings_get_default(); //Configuraci�n de la librar�a
    g_object_set(default_settings, "gtk-button-images", TRUE, NULL); //Configuraci�n de la librar�a
    ...
    gtk_main(); //Definici�n del loop del programa principal main.
...
void FNconfigFormJuego(){ //Configura la pantalla de juego
    ...
    //Creaci�n de la pantalla de juego
  ...
  //  gtk_window_set_resizable(GTK_WINDOW(formJuego.Pantalla), FALSE);
    ...
    //Llamada a funcion para la creaci�n de los men�s de la pantalla de juego
    FNcrearMenusFormJuego();

    gtk_widget_show_all(formJuego.Layout); //Se muestra todo lo que hay en dicha pantalla.
 }

void FNcrearMenusFormJuego(){ //Crea los men�s de opciones dentro de la pantalla de juego
...
void FNcargaCoordenadasTablero(){ //Carga las coordenadas X e Y del layout en una matriz para manejar la colocaci�n gr�fica de las piezas
...
void manejadorTurnoJ1(GtkToggleButton *turno){ //Maneja el turno del Jugador 1 cuando se presiona el boton de tipo ToggleButton, dicho bot�n tiene dos estados (Presionado y No presionado)
...
void manejadorTurnoJ2(GtkToggleButton *turno){ //Maneja el turno del Jugador 2 cuando se presiona el boton de tipo ToggleButton, dicho bot�n tiene dos estados (Presionado y No presionado)
...
void FNdimensionarPiezas(){ //Dimensiona el tama�o de las piezas
...
void FNcolocarPiezas(int PiezaAletoria, int Fila, int Columna){ //Cuando se llama a FNiniciarPiezasJ1 y FNiniciarPiezasJ2, esta funci�n recibe una pieza aleatoria y la coloca tambi�n en una posici�n aleatoria del tablero.
    switch(piezaRandom){
        case ELEFANTEJ1:
...
void FNcrearMenusPiezas(){ //Crea los men�s de cada pieza, los cuales se despliegan al hacer dist�ntos clicks en ellas (MOVER Y EMPUJARESTIRAR)
...
void FNconectarMenusPiezas(){ //Conecta los Men�s creados a las piezas
    //Jugador 1
...
void FNmostrarOPmenusPiezas(){ //(DEPRECADO) Muestra las opciones de los men�s
...
void FNconectarOPMovimientosPiezas(){ //Conecta los submen�s para el movimiento a los men�s de cada pieza
        ///ABAJO
        ///Mover
        //Jugador 1
...
void FNmoverAbajo(GtkWidget *pieza){ //Mueve la pieza que recibe una posici�n abajo
...
void FNmoverArriba(GtkWidget *pieza){ //Mueve la pieza que recibe una posici�n arriba
...
void FNmoverDerecha(GtkWidget *pieza){ //Mueve la pieza que recibe una posici�n a la derecha
...
void FNmoverIzquierda(GtkWidget *pieza){ //Mueve la pieza que recibe una posici�n a la izquierda
...
void FNempujarAbajoJ2(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia abajo
...
void FNestirarArribaJ2(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia arriba
...
void FNempujarDerechaJ2(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia la derecha
...
void FNempujarIzquierdaJ2(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia la izquierda
...
void FNempujarArribaJ1(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia abajo
...
void FNestirarAbajoJ1(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia arriba
...
void FNempujarDerechaJ1(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia la derecha
...
void FNempujarIzquierdaJ1(GtkWidget *pieza){ //Empuja la pieza que recibe una posici�n hacia la izquierda
...
int siCongela(int I, int J){ //Si retorna true, la pieza esta congelada y se env�a una advertencia, si retorna false, se mueve la pieza
